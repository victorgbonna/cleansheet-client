...
export default function EnterChatModal({onNext}) {
    ...
    const {mutate:loadSeason,data:ues, isLoading:loadLoading}=useMutation({
    ...
    const addSeasonQuery= async()=>{
        return await postData(API_ENDPOINTS.GET_NEW_SEASON_INFO,{...msgObj, league:msgObj.league.toLowerCase()})
    }
    const {mutate:addSeasonToDb, isLoading:addLoading}=useMutation({
        addSeasonQuery,
        onError:({error})=>{
            consolelog({error})
            return NotifyError(error.message || 'Could not get data')
    ...
    useEffect(() => {
        if(!msgObj.medium || !msgObj.contact) return
        return setMsgObj({...msgObj, contact:""})
    }, [msgObj.medium]);
        ...
        <ModalLayout onClose={()=>setShowModal('')}>
            <div 
               ...
               <div className="flex justify-between items-center mb-8">
                   ...
                   <img className="cursor-pointer" onClick={()=>setShowModal('')} src={'/svg/close.svg'} alt="icon" />
               </div>
